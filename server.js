...
💰 *Amount Paid:* KES ${order.amount_payable.toFixed(2)}
📲 *Airtime Recipient:* ${recipient}
🏷️ *MPesa Code:* ${tx || 'Processing...'}
📦 *Order:* ${order.order_no}

⏳ Please wait while we process your airtime delivery...
                  `.trim();
...
✅ KES ${order.amount.toFixed(2)} airtime has been successfully sent to *${recipient}*

📦 *Order:* ${order.order_no}
🏷️ *MPesa Code:* ${tx || 'N/A'}

Thank you for using ${clientData.botName || SETTINGS.bot_name || 'FY Bot'}! 🙏
                    `.trim();
...
💰 Payment received but airtime delivery failed.
📲 *Recipient:* ${recipient}
📦 *Order:* ${order.order_no}
...
Please try again or contact support if you need help.
                    ...
                    await alertAdmin(`❌ Payment failed for ${order.order_no}`);
...
If you completed the payment, it may take a few minutes to process. You can check your order status by typing your order number.

If payment was not completed, please try again.
                    `.trim();
...
Thank you for using ${SETTINGS.bot_name || 'FY Bot'}! 🙏
          `.trim();
          await notifyUser(clientId, userJid, deliveryMsg);
        } else {
          const failMsg = `
